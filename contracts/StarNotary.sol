...
contract StarNotary is ERC721 {
    ...
    function lookUptokenIdToStarInfo (uint _tokenId) public view returns (string memory) {
        ...
        Star memory starFromToken = tokenIdToStarInfo[_tokenId];
        return starFromToken.name;
    }

    // Implement Task 1 Exchange Stars function
    function exchangeStars(uint256 _tokenId1, uint256 _tokenId2) public {
        // Passing to star tokenId you will need to check if the owner of _tokenId1 or _tokenId2 is the sender
        require(ownerOf(_tokenId1) == msg.sender || ownerOf(_tokenId2) == msg.sender, "Owner must own one of the stars being swapped");
    ...
    function transferStar(address _to1, uint256 _tokenId) public {
        ...
        require(ownerOf(_tokenId) == msg.sender, "Token ID not owned by sender");

        //2. Use the transferFrom(from, to, tokenId); function to transfer the Star
        address sender = msg.sender;
        address receiver = _to1;
        transferFrom(sender, receiver, _tokenId);
    }
